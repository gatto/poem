...
def train_black_box(
    ...
    if black_box in ["RF", "AB"]:
        ...
        def bb_fit(X, Y):
            ...
            model.fit(
                X, Y, epochs=100, batch_size=128, validation_split=0.3, verbose=True
            )
            return model
...
def get_autoencoder(X, ae_name, dataset, path_aemodels, latent_dim=False):
    ...
    if ae_name == "aae":
        if dataset in ["mnist", "fashion", "cifar10bw", "emnist", "afromnist_e"]:
            ...
            ae = AdversarialAutoencoderCifar10(
                shape=shape,
                input_dim=input_dim,
                latent_dim=latent_dim,
                hidden_dim=128,
...
def store_fcpn(
    ...
    jrow_fcp = {
        ...
        "fidelity": fidelity,
        "compactness": compact,
        "compactness_var": compact_var,
        "lcompactness": lcompact,
        "lcompactness_var": lcompact_var,
        "time": run_time,
    }
